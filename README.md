...
9. [Referrals & invites](#referrals--invites)
10. [Running the app](#running-the-app)
11. [Testing](#testing)
12. [Building a release APK / AAB](#building-a-release-apk--aab)
12. [Roadmap](#roadmap)
...
2. Go to **SQL Editor** → paste the contents of `supabase/schema.sql` → **Run**.
   ...
   - A unique per-user `referral_code` and the `get_referrer(code)` / `redeem_referral(code)` RPCs that power invite links (see [Referrals](#referrals--invites))
...
ScrollIQ uses **Supabase-backed referral URLs** to invite friends. Each user is
assigned a unique `referral_code` (column on `public.users`, generated by the
`handle_new_user` trigger). The flow:

1. **Share** — tapping *Invite* (Friends screen app-bar) or *Challenge Your
   Friend* (post-login welcome screen, `/welcome-invite`) opens the system
   share sheet with a link:
   ```
   https://scrolliq.app/invite?ref=<CODE>
   ```
   The base URL is configurable via `REFERRAL_BASE_URL` in `.env`.
2. **Open** — when an invited user opens the link, `app_links` delivers the URI
   to `ReferralService`, which extracts `ref` and stores it locally
   (`pending_referral_code`) until the user authenticates.
3. **Redeem** — once signed in, `ReferralService` calls the `redeem_referral`
   RPC. This security-definer function creates a **pending friend request** from
   the referrer to the new user (bypassing the `Friends: insert sender` RLS
   policy safely, idempotently, and with self-referral protection).

Relevant pieces:

| Piece | Location |
|-------|----------|
| Code column + RPCs | `supabase/schema.sql` (`referral_code`, `get_referrer`, `redeem_referral`) |
| Repository | `lib/features/referral/data/referral_repository.dart` |
| Service (links + redeem + share) | `lib/features/referral/referral_service.dart` |
| Deep-link intent filters | `android/app/src/main/AndroidManifest.xml` |

### Deep-link setup

- **Custom scheme** (`scrolliq://invite?ref=CODE`) works out of the box and is
  ideal for local testing:
  ```bash
  adb shell am start -a android.intent.action.VIEW \
      -d "scrolliq://invite?ref=ABCD1234"
  ```
- **HTTPS App Links** (`https://scrolliq.app/invite?ref=CODE`) require hosting a
  Digital Asset Links file at
  `https://scrolliq.app/.well-known/assetlinks.json` and an iOS
  `apple-app-site-association` file. Until those are published, https links open
  in the browser rather than the app. Point `REFERRAL_BASE_URL` at a domain you
  control.
...
The first launch shows the splash → onboarding (story → permissions → demo) → login. After sign-in, a one-time **Challenge Friends** welcome screen (`/welcome-invite`) is shown so the shared invite URL carries the new user's real `?ref=<CODE>` (the referral code is created by the `handle_new_user` Postgres trigger when Supabase auth creates the row, so this prompt has to live post-login). Tapping either *Challenge Your Friend* or *I'll Do It Later* persists `post_login_invite_shown=true` and the router redirects on to `/home`.
